                                  ...
                                  sidebarLayout(
                                    sidebarPanel(
                                      ...
                                      sliderInput("slider0", "Select a Time Unit Range",
                                                  min = 1980, max = 2020, value = c(2004, 2017), step = 5
                                                  ),
                                 ...
                                 sidebarLayout(
                                   sidebarPanel(
                                     ...
                                     sliderInput("slider", "Select a Time Unit Range",
                                                 min = 1980, max = 2020, value = c(2004, 2017)),
                                     br(),
                                   ...
                                   mainPanel(
                                     ...
                                     plotlyOutput("ea_plot_1", height = 400),
                                     br(),
                                     plotlyOutput("ea_plot_2", height = 400),
                                     br(),
                                     plotlyOutput("ea_plot_3", height = 400),
                                     br(),
                                     plotlyOutput("ea_plot_4", height = 400),
                                     br()
                                   )
                                 )
                        ),
...
server <- function(input, output) { 
  ...
  output$ea_plot_1 <- renderPlotly({
    ...
    d1 <- ggplot(dat4, aes(x=as.factor(u_Time), y=prevalence)) + 
      geom_jitter(aes(label=u_Loc), width = 0.2, alpha = 0.3) +
  ...
  output$ea_plot_3 <- renderPlotly({
    ...
    d3 <- ggplot(dat4, aes(x=u_Time, y=prevalence)) + 
      geom_jitter(aes(label=u_Loc), width = 0.2, alpha = 0.2) +
      geom_smooth(se=TRUE, level=0.99) +
      theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", hjust=0)) +
      labs(title = "Prevalence Over Time") + labs(x = "Time Unit") + labs(y = "Prevalence") +
      scale_x_continuous(breaks=year_m:2020)

    (gg3 <- ggplotly(d3))
  })

  output$ea_plot_4 <- renderPlotly({
    ##do a bunch of processes on data to calculate interquartile range (iqr), standard deviation (sd), mean of total population (mean)
    ### and mean of patient within each u_Time (meanP)
    ...
    d4 <- ggplot(dat4, aes(x=u_Time, y=population)) + 
      geom_jitter(aes(label=u_Loc), width = 0.2, alpha = 0.3) +
      geom_smooth(se=TRUE, level=0.99) +
      theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", hjust=0)) +
      labs(title = "Overall Patient Population Over Time") + labs(x = "Time Unit") + labs(y = "Overall Patient Population") +
      scale_x_continuous(breaks=year_m:2020)
...
#    p <- ggplot(dat0(), aes(x=as.factor(u_Time), y=prevalence, color=u_Loc)) + 
#      geom_boxplot() +
#      geom_jitter() +
#      geom_point() +
...
options(shiny.port = CHANGE_PORT)
options(shiny.host = "CHANGE_HOST")
shinyApp(ui, server)
